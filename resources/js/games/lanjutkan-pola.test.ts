...
function play(params: Record<string, unknown>, correct: boolean, elapsedMs = 0) {
    ...
    while (!mod.isFinished() && guard++ < 100) {
        ...
        if (!round) break;
        mod.onAnswer({ value: correct ? round.answer : `__wrong__${round.answer}`, elapsedMs });
    }
...
describe('lanjutkan-pola generator is deterministic & verifiable', () => {
    ...
    it.each(types)('every generated %s puzzle has exactly one verifiable answer', (type) => {
    ...
    it('falls back to a safe default when pattern_types is empty', () => {
        expect(settingsFor({ pattern_types: [] }).patternTypes).toEqual(['arithmetic', 'shape']);
    });
});

describe('lanjutkan-pola scoring (correct × speed)', () => {
    const base = { pattern_types: ['arithmetic'], sequence_length: 4, options_count: 3, rounds: 8 };
