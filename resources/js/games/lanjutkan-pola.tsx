...
export function readSettings(params: GameParams): PolaSettings {
    ...
    return {
        ...
        sequenceLength: Math.min(8, Math.max(3, Math.floor(toNumber(params.sequence_length, 4)))),
        optionsCount: Math.min(6, Math.max(2, Math.floor(toNumber(params.options_count, 3)))),
        rounds: Math.min(30, Math.max(1, Math.floor(toNumber(params.rounds ?? params.total_questions, 10)))),
    };
}
...
function buildOptions(answer: string, distractors: string[], optionsCount: number): string[] {
    const set = new Set<string>([answer]);
    for (const d of distractors) {
        if (set.size >= optionsCount) break;
        set.add(d);
    }
    return shuffle([...set]);
}

function generateNumeric(type: 'arithmetic' | 'geometric', settings: PolaSettings): PolaRound {
    ...
    if (type === 'arithmetic') {
        ...
        const d = randInt(1, 5);
        for (let i = 0; i < len; i++) nums.push(start + i * d);
        answer = start + len * d;
        distractCandidates.push(answer + 1, answer - 1, answer + d, answer - d, answer + 2 * d, nums[len - 1]);
    } else {
        const start = randInt(2, 4);
        const r = randInt(2, 3);
        for (let i = 0; i < len; i++) nums.push(start * r ** i);
        answer = start * r ** len;
        const last = nums[len - 1];
        distractCandidates.push(answer + 1, answer - 1, last, answer + last, answer - last, answer * 2);
    }
    ...
    while (distractors.length < settings.optionsCount - 1) {
        const candidate = answer + randInt(1, 9) * (Math.random() < 0.5 ? 1 : -1);
    ...
    return {
        ...
        options: buildOptions(String(answer), distractors, settings.optionsCount),
        index: 0,
        total: 0,
    };
}

function generateCyclic(type: 'shape' | 'color', settings: PolaSettings): PolaRound {
    ...
    for (let i = 0; i < len; i++) sequence.push(cycle[i % period]);
    const answer = cycle[len % period];

    const distractors = shuffle(pool.filter((k) => k !== answer)).slice(0, settings.optionsCount - 1);
 ...
 */
export function verifyQuestion(round: PolaRound, optionsCount: number): boolean {
    let expected: string;

    if (round.type === 'arithmetic') {
        const seq = round.sequence.map(Number);
        const d = seq[1] - seq[0];
        if (!seq.every((v, i) => i === 0 || v - seq[i - 1] === d)) return false;
        expected = String(seq[seq.length - 1] + d);
    } else if (round.type === 'geometric') {
        const seq = round.sequence.map(Number);
        const r = seq[1] / seq[0];
        if (!seq.every((v, i) => i === 0 || v / seq[i - 1] === r)) return false;
    ...
    if (expected !== round.answer) return false;
    if (round.options.length !== optionsCount) return false;
    if (new Set(round.options).size !== round.options.length) return false;
    return round.options.filter((o) => o === round.answer).length === 1;
}

export function createModule(params: GameParams): GameModule<PolaRound, PolaAnswer> {
    ...
    return {
        ...
        onAnswer(answer) {
            const isCorrect = current !== null && answer.value === current.answer;

            if (isCorrect) {
                correct += 1;
                const fraction = Math.max(0, Math.min(1, 1 - answer.elapsedMs / SPEED_REF_MS));
...
function Token({ kind, token, large }: { kind: Kind; token: string; large?: boolean }) {
    if (kind === 'number') {
        return <span className={cn('font-bold tabular-nums', large ? 'text-3xl' : 'text-2xl')}>{token}</span>;
    }
    if (kind === 'shape') {
        return (
            <span className={large ? 'text-4xl' : 'text-3xl'} aria-label={token}>
                {SHAPES[token]}
            </span>
        );
    }
...
function PolaBoard({ round, onAnswer, disabled }: GameRoundProps<PolaRound, PolaAnswer>) {
